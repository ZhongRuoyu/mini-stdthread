...
//  B minithread::Thread's constructor should properly handle exceptions and not
//  leak
//    memory.
// Plan:
...
int main(int, char **) {
    ...
    G::op_run = false;
#ifndef ASAN_IS_ON
// The test below expects `minithread::Thread` to call `new`, which may not be
// the case for all implementations.
    ...
    {
        minithread::Thread t = minithread::Thread(MoveOnly(), MoveOnly());
        t.join();
    }
